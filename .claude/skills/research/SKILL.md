...
### 3-1. 결과 확인 및 중복 처리
1. 서브에이전트가 반환한 파일 경로 목록을 수집
2. 각 파일의 `url:` 또는 `title:`을 추출하여, vault 기존 노트(`~/dapi-ssot/research/`)에 같은 URL/제목이 이미 있는지 grep으로 확인
3. **URL 중복 발견 시**:
   - 새 노트와 기존 노트를 비교 (날짜, 버전, 핵심 내용)
   - **내용 변경 있으면**: 기존 노트를 새 내용으로 업데이트 + 프론트매터에 `updated: {date}` 추가. 새 파일 삭제
   - **내용 변경 없으면**: 새 파일만 삭제. 기존 노트 유지
   - 허브 노트에는 기존 노트 파일명으로 링크
4. **새 자료만** 읽고 내용 확인
5. 관련도 낮은 항목 제거 (파일 삭제)
...
- 기존 토픽과 중복 → 기존 허브 노트에 새로운 결과를 append, 개별 노트는 URL/제목 기준 중복이면 생성하지 않음
