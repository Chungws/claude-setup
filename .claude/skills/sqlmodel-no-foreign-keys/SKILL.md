...
description: SQLModel database modeling for Dudaji Dashboard with strict NO Foreign Keys policy (ADR-001). Use when defining database models, creating migrations, handling table relationships, or reviewing database code. Provides patterns for index-based references and service-layer data integrity management.
---
...
This skill provides SQLModel patterns for Dudaji Dashboard, which **absolutely prohibits** database-level Foreign Key constraints per ADR-001. Use this skill when working with database models, migrations, or any code that references relationships between tables.
